...
``create_app``\이 응용 팩토리 함수이다. 따라하기 이후 부분에서
...
#.  :meth:`app.config.from_pyfile() <Config.from_pyfile>`\로
    ...
    값들로 기본 설정을 바꾼다. 예를 들어 배치 시에 이를 이용해
    진짜 ``SECRET_KEY``\를 설정할 수 있다.

    *   팩토리로 ``test_config``\를 줘서 인스턴스 설정 대신
        쓰이게 할 수 있다. 따라하기 이후에서 작성하게 될
...
#.  :meth:`@app.route() <Flask.route>`\로 따라하기 나머지 부분으로
...
잊지 말자. ``flaskr`` 패키지가 아니라 최상위 ``flask-tutorial``
디렉터리에서다.

개발 모드에서는 페이지에서 예외를 던질 때마다 대화형 디버거가
나타나고 코드를 변경할 때마다 서버가 재시작된다. 계속 돌게 두고서
따라하기를 따라가며 브라우저 페이지만 새로고침 하면 된다.
