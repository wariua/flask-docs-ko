...
이 길라잡이에서는 Flaskr라는 간단한 블로그 응용을 만드는 과정을
...
출발점으로 삼기에는 좋지만 길라잡이에서 플라스크의 모든 기능을
다루지는 않는다. :ref:`quickstart` 에서 플라스크가 뭘 할 수 있는지
둘러본 다음 문서를 더 들여다 보면 된다. 한편으로 길라잡이에서는
...
도움이 된다. 그래서 길라잡이 앞부분에서 이것저것 갖다 붙여야 할 게
...
:gh:`길라잡이의 프로젝트가 플라스크 저장소에 예시로 있다
<examples/tutorial>`. 길라잡이를 따라가면서 작성한 프로젝트를
