...
#.  :meth:`@bp.route <Blueprint.route>` 로 URL ``/register`` 를
    뷰 함수 ``register`` 로 연결시킨다. 플라스크가 ``/auth/register``
    로 요청을 받으면 ``register`` 뷰를 호출해서 그 반환 값을
    응답으로 쓰게 된다.

#.  사용자가 양식을 제출하면 :attr:`request.method <Request.method>`
    가 ``'POST'`` 가 된다. 그 경우 입력 검증을 시작한다.

#.  :attr:`request.form <Request.form>` 은 특별한 :class:`dict`
    로서 제출받은 양식 키와 값들을 매핑 해 준다. 사용자는 자기
...
#.  검사가 통과하면 새 사용자 데이터를 데이터베이스에 집어넣는다.
    ...
    한다. 대신 :func:`~werkzeug.security.generate_password_hash`
    를 써서 패스워드를 안전하게 해싱 하고 그 해시를 저장한다.
...
예를 들어 길라잡이 초반에 앱 팩토리에 추가했던 ``hello()``
